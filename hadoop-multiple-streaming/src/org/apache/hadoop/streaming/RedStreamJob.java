 ...
 * and monitoring)
 */
public class RedStreamJob implements Tool {

	protected static final Log LOG = LogFactory.getLog(RedStreamJob.class
			.getName());
	 ...
	 */
	@Deprecated
	public RedStreamJob(String[] argv, boolean mayExit) {
		this();
		argv_ = argv;
	...
	static public JobConf createJob(String[] argv) throws IOException {
		RedStreamJob job = new RedStreamJob();
		job.argv_ = argv;
		job.init();
		job.preProcessArgs();
		job.parseArgv();
	...
	private void setupOptions() {
		...
		Option mapper = createOption("mapper", "The streaming command to run",
				"cmd", 1, false);
	...
	public void exitUsage(boolean detailed) {
		...
		System.out
				.println("  -mapper   <cmd|JavaClassName>      The streaming command to run");
		System.out
	...
	protected String packageJobJar() throws IOException {
		...
		if (runtimeClasses == null) {
			runtimeClasses = StreamUtil.findInClasspath(RedStreamJob.class
					.getName());
		}
		if (runtimeClasses == null) {
	...
	protected void setJobConf() throws IOException {
		if (additionalConfSpec_ != null) {
			LOG.warn(dirKey + " warns\t" + "-additionalconfspec option is deprecated, please use -conf instead.");
		...
		if (mapCmd_ != null) {
			// red jobs의 mapper는 모두 "cat" 이다.
			jobConf_.setMapperClass(PipeMapper.class);
			jobConf_.setMapRunnerClass(PipeMapRunner.class);
			jobConf_.set("stream.map.streamprocessor", URLEncoder.encode("cat", "UTF-8"));
		}
	...
	protected void jobInfo() {
		if (isLocalHadoop()) {
			LOG.info(dirKey + " informs\t" + "Job running in-process (local Hadoop)");
		} else {
			String hp = getJobTrackerHostPort();
			LOG.info(dirKey + " informs\t" + "To kill this job, run:");
			LOG.info(dirKey + " informs\t" + getHadoopClientHome()
					+ "/bin/hadoop job  -Dmapred.job.tracker=" + hp + " -kill "
					+ jobId_);
			// LOG.info(dirKey + " informs\t" + "Job file: " + running_.getJobFile());
			LOG.info(dirKey + " informs\t" + "Tracking URL: "
	...
	public int submitAndMonitorJob() throws IOException {
		...
		*/
		...
		//LOG.info(dirKey + " informs\t" + "Running job: " + running_.getID());
		try {
			running_ = jc_.submitJob(jobConf_);
			jobId_ = running_.getID();

			LOG.info(dirKey + " informs\t" + "getLocalDirs(): "
					+ Arrays.asList(jobConf_.getLocalDirs()));
			LOG.info(dirKey + " informs\t" + "Running job: " + jobId_);
			jobInfo();
			
			LOG.info(dirKey + " informs\t" + "Job complete: " + jobId_);
			LOG.info(dirKey + " informs\t" + "Output: " + output_);
			error = false;
		} catch (FileNotFoundException fe) {
			LOG.error(dirKey + " makes error\t" + "Error launching job , bad input path : "
					+ fe.getMessage());
			return 2;
		} catch (InvalidJobConfException je) {
			LOG.error(dirKey + " makes error\t" + "Error launching job , Invalid job conf : "
					+ je.getMessage());
			return 3;
		} catch (FileAlreadyExistsException fae) {
			LOG.error(dirKey + " makes error\t" + "Error launching job , Output path already exists : "
					+ fae.getMessage());
			return 4;
		} catch (IOException ioe) {
			LOG.error(dirKey + " makes error\t" + "Error Launching job : " + ioe.getMessage());
			return 5;
		} finally {
			if (error && (running_ != null)) {
				LOG.info(dirKey + " informs\t" + "killJob...");
				running_.killJob();
			}
			jc_.close();
		}
