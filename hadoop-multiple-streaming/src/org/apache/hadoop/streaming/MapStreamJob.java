...
/**
 * All the client-side work happens here. (Jar packaging, MapRed job submission
 * and monitoring)
 */
public class MapStreamJob implements Tool {

	protected static final Log LOG = LogFactory.getLog(MapStreamJob.class
			.getName());
	...
	/**
	 * @deprecated use StreamJob() with ToolRunner or set the Configuration using
	 *             {@link #setConf(Configuration)} and run with
	 *             {@link #run(String[])}.
	 */
	@Deprecated
	public MapStreamJob(String[] argv, boolean mayExit) {
		this();
		argv_ = argv;
	...
	/**
	 * This method creates a streaming job from the given argument list. The
	 * created object can be used and/or submitted to a jobtracker for execution
	 * by a job agent such as JobControl
	 * 
	 * @param argv
	...
	static public JobConf createJob(String[] argv) throws IOException {
		MapStreamJob job = new MapStreamJob();
		job.argv_ = argv;
		job.init();
		job.preProcessArgs();
		job.parseArgv();
	...
	/**
	 * This is the method that actually intializes the job conf and submits the
	 * job to the jobtracker
	 * 
	 * @throws IOException
	...
	void postProcessArgs() throws IOException {
		...
		if (mapCmd_.size() == 0) {
			fail("Required argument: -mapred <name>");
		}
//		mapCmd_ = unqualifyIfLocalPath(mapCmd_);
		it = mapCmd_.iterator();
		while (it.hasNext()) {
			unqualifyIfLocalPath((String) it.next());
		}
		it = redCmd_.iterator();
		while (it.hasNext()) {
			unqualifyIfLocalPath((String) it.next());
	...
	void parseArgv() {
		...
		if (cmdLine != null) {
			...
			additionalConfSpec_ = (String) cmdLine
					.getOptionValue("additionalconfspec");
			inputFormatSpec_ = (String) cmdLine.getOptionValue("inputformat");
			outputFormatSpec_ = (String) cmdLine.getOptionValue("outputformat");
			numReduceTasksSpec_ = (String) cmdLine
					.getOptionValue("numReduceTasks");
			...
			if (null != car && car.length > 0) {
				...
				for (String s : car) {
					cacheArchives = (cacheArchives == null) ? s : cacheArchives
							+ "," + s;
				}
			}
			...
			if (null != caf && caf.length > 0) {
				...
				for (String s : caf) {
					cacheFiles = (cacheFiles == null) ? s : cacheFiles + ","
							+ s;
				}
			}
	...
	private void setupOptions() {
		...
		Option input = createOption("input",
				"DFS input file(s) for the Map step", "path",
				Integer.MAX_VALUE, true);

		Option output = createOption("output",
				"DFS output directory for the Reduce step", "path", 1, true);
		Option mapper = createOption("mapred", "The streaming command(s) to run",
				"outdir/map cmd/red cmd", Integer.MAX_VALUE, true);
		...
		Option file = createOption("file",
				"File to be shipped in the Job jar file", "file",
				Integer.MAX_VALUE, false);
		Option dfs = createOption("dfs",
				"Optional. Override DFS configuration", "<h:p>|local", 1, false);
		Option jt = createOption("jt",
				"Optional. Override JobTracker configuration", "<h:p>|local",
				1, false);
		Option additionalconfspec = createOption("additionalconfspec",
				"Optional.", "spec", 1, false);
		Option inputformat = createOption("inputformat", "Optional.", "spec",
				1, false);
		Option outputformat = createOption("outputformat", "Optional.", "spec",
				1, false);
		Option partitioner = createOption("partitioner", "Optional.", "spec",
				1, false);
		Option numReduceTasks = createOption("numReduceTasks", "Optional.",
				"spec", 1, false);
		Option inputreader = createOption("inputreader", "Optional.", "spec",
				1, false);
		Option mapDebug = createOption("mapdebug", "Optional.", "spec", 1,
				false);
		Option reduceDebug = createOption("reducedebug", "Optional", "spec", 1,
				false);
		Option jobconf = createOption("jobconf",
				"(n=v) Optional. Add or override a JobConf property.", "spec",
				1, false);

		Option cmdenv = createOption("cmdenv",
				"(n=v) Pass env.var to streaming commands.", "spec", 1, false);
		Option cacheFile = createOption("cacheFile", "File name URI",
				"fileNameURI", Integer.MAX_VALUE, false);
		Option cacheArchive = createOption("cacheArchive", "File name URI",
				"fileNameURI", Integer.MAX_VALUE, false);
		...
		allOptions = new Options().addOption(input).addOption(output)
				.addOption(mapper).addOption(combiner).addOption(reducer)
				.addOption(file).addOption(dfs).addOption(jt)
				.addOption(additionalconfspec).addOption(inputformat)
				.addOption(outputformat).addOption(partitioner)
				.addOption(numReduceTasks).addOption(inputreader)
				.addOption(mapDebug).addOption(reduceDebug).addOption(jobconf)
				.addOption(cmdenv).addOption(cacheFile).addOption(cacheArchive)
				.addOption(verbose).addOption(info).addOption(debug)
				.addOption(inputtagged).addOption(help);
	}
	...
	public void exitUsage(boolean detailed) {
		...
		System.out
				.println("          $HADOOP_HOME/hadoop-streaming.jar [options]");
		System.out.println("Options:");
		System.out
				.println("  -input    <path>     DFS input file(s) for the Map step");
		System.out
				.println("  -output   <path>     DFS output directory for the Reduce step");
		System.out
				.println("  -mapper   <cmd>      The streaming command(s) to run");
		System.out
				.println("  -combiner <JavaClassName> Combiner has to be a Java class");
		System.out
				.println("  -reducer  <cmd|JavaClassName>      The streaming command to run");
		System.out
				.println("  -file     <file>     File/dir to be shipped in the Job jar file");
		System.out
				.println("  -inputformat TextInputFormat(default)|SequenceFileAsTextInputFormat|JavaClassName Optional.");
		System.out
				.println("  -outputformat TextOutputFormat(default)|JavaClassName  Optional.");
		...
		System.out
				.println("  -cmdenv   <n>=<v>    Optional. Pass env.var to streaming commands");
		...
		if (!detailed) {
			...
			System.out
					.println("Use $HADOOP_HOME/bin/hadoop jar build/hadoop-streaming.jar -info");
			fail("");
		}
		System.out.println();
		System.out
				.println("In -input: globbing on <path> is supported and can have multiple -input");
		System.out
				.println("Default Map input format: a line is a record in UTF-8");
		System.out
				.println("  the key part ends at first TAB, the rest of the line is the value");
		System.out
				.println("Custom input format: -inputformat package.MyInputFormat ");
		System.out.println("Map output format, reduce input/output format:");
		System.out
				.println("  Format defined by what the mapper command outputs. Line-oriented");
		System.out.println();
		System.out
				.println("The files named in the -file argument[s] end up in the");
		System.out
				.println("  working directory when the mapper and reducer are run.");
		System.out
				.println("  The location of this working directory is unspecified.");
		System.out.println();
		System.out
				.println("To set the number of reduce tasks (num. of output files):");
		System.out.println("  -D mapred.reduce.tasks=10");
		System.out
				.println("To skip the sort/combine/shuffle/sort/reduce step:");
		...
		System.out
				.println("  because the input filename and the map input order are preserved");
		...
		System.out
				.println("  -D mapred.reduce.tasks.speculative.execution=true");
		System.out
				.println("To name the job (appears in the JobTracker Web UI):");
		...
		System.out
				.println("Additional local temp directories with -cluster local:");
		...
		System.out
				.println("To treat tasks with non-zero exit status as SUCCEDED:");
		System.out.println("  -D stream.non.zero.exit.is.failure=false");
		System.out
				.println("Use a custom hadoopStreaming build along a standard hadoop install:");
		System.out
				.println("  $HADOOP_HOME/bin/hadoop jar /path/my-hadoop-streaming.jar [...]\\");
		System.out
		...
		System.out
				.println("To set an environement variable in a streaming command:");
		System.out
				.println("   -cmdenv EXAMPLE_DIR=/home/example/dictionaries/");
		System.out.println();
		...
		System.out.println();
		System.out
				.println("Example: $HSTREAMING -mapper \"/usr/local/bin/perl5 filter.pl\"");
		System.out
				.println("           -file /local/filter.pl -input \"/logs/0604*/*\" [...]");
		System.out
				.println("  Ships a script, invokes the non-shipped perl interpreter");
		System.out
				.println("  Shipped files go to the working directory so filter.pl is found by perl");
		System.out
				.println("  Input files are all the daily logs for days in month 2006-04");
		fail("");
	}
	...
	protected String packageJobJar() throws IOException {
		...
		// build/hadoop-<version>-dev-streaming.jar
		...
		// where findInClasspath() would find the version of
		// hadoop-streaming.jar in $HADOOP_HOME
		String runtimeClasses = config_.get("stream.shipped.hadoopstreaming"); // jar
																				// or
																				// class
																				// dir
		...
		if (runtimeClasses == null) {
			throw new IOException("runtime classes not found: "
					+ getClass().getPackage());
		} else {
			msg("Found runtime classes in: " + runtimeClasses);
		}
		if (isLocalHadoop()) {
			// don't package class files (they might get unpackaged in "." and
			// then
			// hide the intended CLASSPATH entry)
			// we still package everything else (so that scripts and executable
			// are found in
		...
		System.out.println("packageJobJar: " + packageFiles_ + " " + unjarFiles
				+ " " + jobJar + " tmpDir=" + tmpDir);
		if (debug_ == 0) {
			jobJar.deleteOnExit();
		}
	...
	protected void setJobConf() throws IOException {
		...
		// (mapred.working.dir will be lazily initialized ONCE and depends on
		// FS)
		...
		if (inReaderSpec_ == null && inputFormatSpec_ == null) {
			...
			if (inputFormatSpec_.equals(TextInputFormat.class.getName())
					|| inputFormatSpec_.equals(TextInputFormat.class
							.getCanonicalName())
					|| inputFormatSpec_.equals(TextInputFormat.class
							.getSimpleName())) {
				fmt = TextInputFormat.class;
					...
					.equals(SequenceFileAsTextInputFormat.class.getName())
					|| inputFormatSpec_
							.equals(SequenceFileAsTextInputFormat.class
									.getCanonicalName())
					|| inputFormatSpec_
							.equals(SequenceFileAsTextInputFormat.class
									.getSimpleName())) {
		...
		if (dirKey_ != null) {
			Iterator it = dirKey_.iterator();
			int idx = 0;
			
			while ( it.hasNext() ) {
				String key = (String) it.next();
				jobConf_.set("stream.dir.key" + String.valueOf(idx),
		...
		if (mapCmd_ != null) {
			Iterator it = mapCmd_.iterator();
			int idx = 0;
			
			jobConf_.setMapperClass(MultiPipeMapper.class);
			jobConf_.setMapRunnerClass(MultiPipeMapRunner.class);
			
			while ( it.hasNext() ) {
				String mapper = (String) it.next();
				jobConf_.set("stream.map.streamprocessor" + String.valueOf(idx),
						URLEncoder.encode(mapper, "UTF-8"));		
				idx++;
			}
			jobConf_.set("stream.map.num.streamprocessors", URLEncoder.encode(String.valueOf(idx), "UTF-8"));
		}
		...
		// 첫번째 map job의 reducer는 무조건 NONE이다
		boolean reducerNone_ = true;
		jobConf_.set("stream.reduce.streamprocessor", URLEncoder.encode("NONE", "UTF-8"));
		// 두번째 red jobs들을 위한 설정
		if (redCmd_ != null) {
			Iterator it = redCmd_.iterator();
			int idx = 0;
			
			while ( it.hasNext() ) {
				String reducer = (String) it.next();
				jobConf_.setReducerClass(PipeReducer.class);
				jobConf_.set("stream.reduce.streamprocessor"+ String.valueOf(idx),
							URLEncoder.encode(reducer, "UTF-8"));
				idx++;
			}
			jobConf_.set("stream.reduce.num.streamprocessors", URLEncoder.encode(String.valueOf(idx), "UTF-8"));
			/*
			...
			if (redCmd_.compareToIgnoreCase("aggregate") == 0) {
				...
				c = StreamUtil.goodClassOrNull(jobConf_, redCmd_,
						defaultPackage);
				if (c != null) {
		...
		if (inReaderSpec_ != null) {
			...
			c = StreamUtil.goodClassOrNull(jobConf_, readerClass,
					defaultPackage);
			if (c != null) {
...
//		jobConf_.setOutputFormat(fmt);
		jobConf_.setOutputFormat(StreamUtil.goodClassOrNull(jobConf_, "org.apache.hadoop.streaming.PartitionByKeyField",
				defaultPackage));
		
		// multipleoutputs
		MultipleOutputs.addNamedOutput(jobConf_, "fromJava", PartitionByKeyField.class, Text.class, Text.class);
	...
	protected void jobInfo() {
		if (isLocalHadoop()) {
			...
			LOG.info(getHadoopClientHome()
					+ "/bin/hadoop job  -Dmapred.job.tracker=" + hp + " -kill "
					+ jobId_);
			// LOG.info("Job file: " + running_.getJobFile());
			LOG.info("Tracking URL: "
					+ StreamUtil.qualifyHost(running_.getTrackingURL()));
		}
	}
	...
	public int submitAndMonitorJob() throws IOException {
		...
		if (jar_ != null && isLocalHadoop()) {
			// getAbs became required when shell and subvm have different
			// working dirs...
		...
		try {
			...
			LOG.info("getLocalDirs(): "
					+ Arrays.asList(jobConf_.getLocalDirs()));
...
//						+ Math.round(running_.reduceProgress() * 100) + "%";
			...
			LOG.error("Error launching job , bad input path : "
					+ fe.getMessage());
			return 2;
		} catch (InvalidJobConfException je) {
			LOG.error("Error launching job , Invalid job conf : "
					+ je.getMessage());
			return 3;
	...
	protected ArrayList dirKey_ = new ArrayList();
	//	protected String mapCmd_;
	protected ArrayList mapCmd_ = new ArrayList();
	protected ArrayList redCmd_ = new ArrayList();
	...
	// Use to communicate config to the external processes (ex
	// env.var.HADOOP_USER)
	// encoding "a=b c=d"
